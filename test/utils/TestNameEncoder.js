...
contract('UniversalResolver', function () {
  ...
  describe('encodeName()', () => {
    it('should encode a name', async () => {
      const result = await testNameEncoder.encodeName('foo.planq')
      expect(result['0']).to.equal(dns.hexEncodeName('foo.planq'))
      expect(result['1']).to.equal(namehash('foo.planq'))
    })
    ...
    it('should encode a long name', async () => {
      const result = await testNameEncoder.encodeName('something.else.test.planq')
      expect(result['0']).to.equal(dns.hexEncodeName('something.else.test.planq'))
      expect(result['1']).to.equal(namehash('something.else.test.planq'))
    })
  })
})
